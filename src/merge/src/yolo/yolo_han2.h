        ...
        public:
            ...
            */
            virtual std::vector<han2> worker(std::vector<cv::Mat>& img) 
            {
                ...
                {
                    cv::Mat resized;
                    cv::resize(img[i], resized, cv::Size(64,64));
                    resizeds.push_back(resized);
                }
                
                //std::cout<< "input_shape_[2]h: "<<input_shape_[2] << " 64w: "<< 64<<std::endl;
        ...
        protected:
            ...
            */
            ...
            virtual ov::Tensor preprocess(std::vector<cv::Mat>& image) 
            {
                ...
                unsigned long num_roi = image.size();
                ov::Shape input_shape = {1, num_roi, 3, 64, 64};
                ov::Tensor input_tensor(ov::element::f32, input_shape);
                // 将数据复制到张量
                float* tensor_data = input_tensor.data<float>();
                for(int c = 0; c < num_roi*3; ++c) {
                    memcpy(tensor_data + c*64 * 64,
                        changes[c].data,
                        64 * 64 * sizeof(float));
                }
                return input_tensor;
            ...
            virtual std::vector<han2> postprocess(ov::Tensor& output) 
            {
                ...
                const int num_image = shape[1];
                const int cls = shape[2];

                std::vector<std::vector<float>> channels(num_image);
